 ...
 *
 * There should be no need to change this for task A. If you need to make
 * changes for task B, please make a copy, then modify the copy for task B.
 *
...
public class RmitCovidModelling {
	...
	public static void usage(String progName) {
		System.err.println(progName
				+ ": [-f <filename to load graph>] [-o <filename to store output>]<implementation>] <implementation>");
		System.err.println("<implementation> = <adjlist | adjmat | incmat>");
		System.err.println(
				"If the -o optional output filename is specified, then non-interative mode will be used and output is written to the specified.  Otherwise interative mode is assumed and output is written to System.out.");
		System.exit(1);
	} // end of usage()
	...
	/**
	 * Process the operation commands coming from inReader, and updates the graph
		...
		while (!bQuit && (line = inReader.readLine()) != null) {
			...
			try {
				...
				case "SIR":
					...
					break;
				case "TIME":
					System.out.println("Total Execution time in nanoseconds: " + t.getTotal());
					System.out.println("Average Execution time in nanoseconds: " + t.getAvg());
					break;
				// quit
				case "Q":
					bQuit = true;
	...
	/**
	 * Main method. Determines which implementation to test and processes command
	 * line arguments.
	 */
	public static void main(String[] args) {
		...
		// -f <inputFilename> specifies the file that contains edge list information to
